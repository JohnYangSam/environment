...
# A replacement for: ~/.vim/bundle/YouCompleteMe/third_party/ycmd/build.sh
...
function python_finder {
  ...
  # #########################################################
  # IMPORTANT: Fix to work with pyenv
  # See: https://github.com/Valloric/YouCompleteMe/issues/8
  # #########################################################
  python_prefix=$(python-config --prefix | sed 's/^[ \t]*//')
  python_library="${python_prefix}/lib/python2.7/config"
  pythong_include="${python_prefix}/lib/python2.7/site-packages"
